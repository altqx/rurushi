...
fn build_ffmpeg_command(input_path: &Path, output_dir: &Path) -> Command {
    ...
    cmd.arg("-re")
        ...
        .args(["-hls_flags", "append_list+delete_segments+program_date_time+omit_endlist+independent_segments"])
...
async fn cleanup_hls_directory(out_dir: &Path) -> Result<(), String> {
    if out_dir.exists() {
        ...
        fs::remove_dir_all(out_dir).await.map_err(|e| format!("Failed to remove HLS directory: {}", e))?;
    }
    fs::create_dir_all(out_dir).await.map_err(|e| format!("Failed to create HLS directory: {}", e))?;
...
async fn process_episode(
    ...
    let mut cmd = build_ffmpeg_command(file_path, out_dir);
...
async fn process_video_file(file: &Path, out_dir: &Path) -> Result<(), String> {
    if !file.exists() {
    ...
    let mut cmd = build_ffmpeg_command(file, out_dir);
            ...
            if !playlist.is_empty() {
                ...
                for (i, item) in playlist.iter().enumerate() {
                    ...
                    if let Some(episodes) = shows.get(&item.show_name) {
                        println!("[tv] Found {} episodes for show '{}'", episodes.len(), item.show_name);
                        ...
                        for episode in episodes_to_play {
                            if item.repeat_count == 0 {
                                ...
                                if let Some(played_eps) = played.get(&item.show_name) {
                                    if played_eps.contains(&episode.id) {
                                        println!("[playlist] Skipping already played episode: {}", episode.name);
                                        continue;
                                    }
                                }
                            }
                            ...
                            match process_episode(episode, item, Arc::clone(&played_episodes), &out_dir).await {
                                Ok(_) => {
                        ...
                        eprintln!("[tv] No episodes found for show '{}'. Please scan for videos first.", item.show_name);
                ...
                for file in &tv_files {
                    ...
                    match process_video_file(file, &out_dir).await {
                        Ok(_) => {
                            // Success
                        }
                        Err(e) => {
                ...
                if playlist.is_empty() && tv_files.is_empty() {
                    ...
                    println!("[tv] Content available but not ready - Playlist: {}, TV files: {}",
                             playlist.len(), tv_files.len());
                }
                time::sleep(Duration::from_secs(5)).await;
            }
        }
